...
(define (mark-antinodes! grid a b)
  (define slope (map - a b))
  (define antinodes (list (map + a slope)
			  (map - b slope)))
  (for-each
   (lambda (antinode)
     (when (apply array-in-bounds? grid antinode)
       (apply array-set! grid #\# antinode)))
   antinodes))
...
(define (mark-all-antinodes! grid)
  ...
  (hash-for-each
   (lambda (key antennas)
     (for-each
      (lambda (antenna-pair)
	(apply mark-antinodes! grid antenna-pair))
      (choose-2 antennas)))
   antenna-locations))
