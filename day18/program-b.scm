...
(define (first-bad-byte port rows cols)
  (define grid (make-array #\. rows cols))
  (let loop ((pos (read-byte-position port)))
    (apply array-set! grid #\# pos)
    (if (bfs-ram-path grid)
	(loop (read-byte-position port))
	pos)))

(define (main)
  (define bad-byte (first-bad-byte (current-input-port) 71 71))
  (display* (cadr bad-byte) "," (car bad-byte))
  (newline))
